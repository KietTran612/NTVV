...
### Phiên 22/04/2026 (Hiện tại) — m8-scene-polish HOÀN THÀNH
...
- **BUG-10 fixed**: Main Camera transform reset `(7.37, 8.93, 0)` → `(1.2, 0.35, -10)`. Game view render world sau fix, không còn solid color.
- **NOTE-13 (renumbered từ NOTE-08 m6b) fixed**: 6 tiles có child `LockOverlay` + SpriteRenderer với `World_Overlay_Tile_Lock_On` sprite. `CropTileView._lockOverlay` field wired trên cả 6 instance. Placeholder asset recreated (5,626 bytes, GUID `cc3abb6e` preserved).
- **Stray GO**: verified clean — HANDOVER 17/04 note outdated, không còn stray CropTile.
...
- **Integration smoke test PASSED**: 0 errors, 0 NullReferenceException, camera works, lock overlay toggles đúng (SetActive on/off verified).
- **⚠️ Pre-existing issue noted (out of scope)**: `tile_r0_c0` soil sprite renders white circle — `World_Tile_Soil_Base_Default.png` GUID đúng nhưng Unity render fallback. Cần investigate riêng.

### Phiên 22/04/2026 — m8-scene-polish spec + Kiro conventions (earlier)
- **`m8-scene-polish` spec WRITTEN** (3 files, 734 lines, 9 tasks).
...
- **Git**: 2 commits (`744a7ad` conventions + `4dbbfae` m8 spec).
...
| 12 | **m8-scene-polish** | 22/04 | 9 |

### 🚀 Bước tiếp theo

1. **Swap LockOverlay asset thật** — khi có sprite đẹp từ prompt (`docs/asset-prompts/2026-04-17-rework/world-overlays.md`), ghi đè `World_Overlay_Tile_Lock_On.png` cùng path. GUID `cc3abb6e` preserved → scene tự update, không cần sửa code.
2. **Fix tile_r0_c0 soil sprite** — `World_Tile_Soil_Base_Default.png` GUID đúng nhưng Unity render white circle fallback. Cần investigate texture import hoặc reimport.
3. **Generate missing sprites** — bộ rework `docs/asset-prompts/2026-04-17-rework/` (bắt đầu `entities-animals.md`).
4. **Manual UI test** — Storage sell flow + Shop buy flow (button interaction cần manual verify trong Editor).
5. **M9 milestone** — TBD sau m8 DONE. Ý tưởng: thêm tile mới với `_isLocked=true`, gameplay content mới, hoặc build/release milestone.
...
| `m8-scene-polish` | BUG-10 Camera (1.2, 0.35, -10) + LockOverlay wire 6 tiles + BUG-08 [Min] + bug-backlog cleanup |
