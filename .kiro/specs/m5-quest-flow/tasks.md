...
- [x] 0. Audit quest data usage và unlock types thực tế
  - [x] 0.0 · Resource Check
    - Không có sprite asset trong task này — SKIP scan
    - NON-BLOCKING
  - [x] 0.4 · Research Agent — Đọc QuestData.cs + QuestDataSO.cs
    ...
    - Output audit report ngắn gọn:
      ...
      AUDIT: QuestUnlockType values in use:
        - None: 3 quests (q_harvest_wheat, q_buy_chicken, q_reach_level_2)
      QuestUnlockType values NOT in use:
        - ShopTab_Animals: Future
        - Building_NewNPC: Future
        - System_Crafting: Future
      AcceptQuest() prerequisite check: MISSING
      HandleUnlock() real behavior: MISSING (chỉ Debug.Log)
      QuestPanelController OnQuestStateChanged: SUBSCRIBED
      ```
...
- [x] 1. Fix QuestManager — AcceptQuest() prerequisite enforcement (BUG-Q2)
  - [x] 1.0 · Resource Check
    - Không có sprite asset trong task này — SKIP scan
    - NON-BLOCKING
  - [x] 1.4 · Script Agent — Sửa QuestManager.cs AcceptQuest()
    - `script-read` → `Assets/_Project/Scripts/Gameplay/Quests/QuestManager.cs`
...
- [x] 2. Verify/fix QuestPanelController refresh behavior (BUG-Q1 + BUG-Q4)
  - [x] 2.0 · Resource Check
    - Không có sprite asset trong task này — SKIP scan
    - NON-BLOCKING
  - [x] 2.4 · Script Agent — Verify và fix QuestPanelController.cs
    - `script-read` → `Assets/_Project/Scripts/UI/Panels/QuestPanelController.cs`
    - Verify 4 điểm:
...
- [x] 3. Implement HandleUnlock() runtime behavior cho unlock types v1 (BUG-Q3)
  - [x] 3.0 · Resource Check
    - Không có sprite asset trong task này — SKIP scan
    - Verify audit từ Task 0 đã xác định `unlockType` đang được dùng trong data
    - NON-BLOCKING
  - [x] 3.4 · Script Agent — Implement HandleUnlock() trong QuestManager.cs
    - `script-read` → `Assets/_Project/Scripts/Gameplay/Quests/QuestManager.cs`
...
- [x] 4. Integration Smoke Test
  ...
  - [x] 4.1 Prerequisite enforcement test
  ...
  - [x] 4.2 Prerequisite → unlock chain test
  ...
  - [x] 4.3 Progress update + UI refresh test
  ...
  - [x] 4.4 Claim reward + complete quest test
