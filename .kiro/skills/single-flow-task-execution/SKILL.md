...
Kiro executes tasks sequentially in a single thread. All work happens in one execution context.

**Rules:**

1. **One active task only** — never work on multiple tasks simultaneously.
2. **One execution thread only** — no parallel dispatch.
3. **No parallel coding subagents** — work through tasks sequentially.
4. **Track progress** by updating `<project-root>/docs/plans/task.md` at each state change (table-only tracker).
5. **Use clear task boundaries** to delineate each unit of work.
...
- Fresh task scope per task (clean boundaries)
...
For each task, prepare a clear brief:
...
- **executing-plans** — Use for batch execution with human checkpoints
